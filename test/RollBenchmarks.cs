...
public static class RollBenchmarks
{
    ...
    ///
    /// <para>Kept from an investigation that is now settled, because it is the evidence for the
    /// settlement. While the mod was built Debug, the shipped postfix measured 7.5 ns more than
    /// the same arithmetic written flat, and this variant existed to tell two candidate
    /// explanations apart: the depth of the call chain, and the assembly boundary -- the mod is
    /// loaded from a stream out of a zip, which is not how the JIT usually meets an assembly.</para>
    ///
    /// <para>Neither. The gap was <c>DisableOptimizations</c>: a Debug assembly has the JIT
    /// switched off for it entirely. Built Release, this variant and every other in the ladder
    /// land within a nanosecond of each other, which is what the numbers in the log now show.</para>
    /// </summary>
    ...
    ///
    /// <para>In Release the answer is: nothing, to within a nanosecond. The ladder of variants
    /// below is kept because a Debug build says otherwise and says it convincingly, and this is
    /// the test that would catch that mistake being made again.</para>
    /// </summary>
    [GameTest]
    public static void WhatTheAbstractionsCost()
    {
        // One discarded measurement first. Whichever variant is timed first in a run comes out
        // high -- tiered compilation promotes the sweep and the postfix during it, and the first
        // patch swap does one-off Harmony work. It was invisible while the mod was built Debug,
        // because nothing was optimized and there was no tier to climb.
        ...
        // Generous, because in Release every variant lands within a nanosecond or so of the
        // others and the differences are near the resolution of the measurement. It is here to
        // catch the benchmark measuring the wrong thing, not to police small changes.
        if (minimal > full * 1.8)
            throw new AssertionException(
